...
- **`create_display`** — entry function that takes a `&mut DisplayRegistry` and `&mut Publisher`, creates a `Display<Hero>` with three template fields, shares the Display, and transfers the `DisplayCap` to the caller.
