...
/** Call the `create` entry function to deploy an Allowlist + AdminCap. */
export function buildCreateAllowlistTx(packageId: string): Transaction {
  const tx = new Transaction();
  tx.moveCall({ target: `${packageId}::allowlist_seal::create` });
  return tx;
}
...
export function buildAddMemberTx(
  ...
  const tx = new Transaction();
  tx.moveCall({
    target: `${packageId}::allowlist_seal::add_member`,
    arguments: [
